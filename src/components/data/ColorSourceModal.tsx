...
export function ColorSourceModal({
  ...
  // Is form valid?
  // Same table: only need sourceTableId + colorColumnSlug
  // Different table: need sourceTableId + matchColumnSlug + colorColumnSlug
  ...
  const performImport = async (config: ColorSourceConfig): Promise<{ updated: number; unknown?: string[] }> => {
    ...
    try {
      ...
      return { updated: json.updated as number, unknown: json.unknown as string[] | undefined };
    } finally {
      setImporting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await performImport(config);

      toast.success(`Source de couleurs connectée — ${result.updated} produit(s) mis à jour`);

      // Show warning for unknown colors
      if (result.unknown && result.unknown.length > 0) {
        toast.warning(`${result.unknown.length} couleur(s) non reconnue(s) : ${result.unknown.join(', ')}`);
      }
  ...
  const handleForceReimport = async () => {
    ...
    setSaving(true);
    try {
      const result = await performImport(currentConfig);
