...
//
// ━━ MANDAT ADF — ALIGNEMENT LCP MOBILE/DESKTOP v2 (audit ADF v2) ━━
// Audit ADF v2 a découvert un bug critique dans le split 2-instances :
// zainArabic (preload:true) + zainLatin (preload:false) partageaient la
// MÊME variable --font-zain → Next.js générait des règles @font-face
// DUPLIQUÉES avec des URLs divergentes (.p. infix pour preloads vs non-.p.
// pour @font-face) → les fichiers préchargés n'étaient JAMAIS matchés par
// le texte → double fetch de 15.8 KiB + preloads morts.
//
// Solution validée (instance unique) :
//   - subsets:["arabic"] uniquement → Next.js génère les règles @font-face
//     pour l'arabe ET le latin (les 2 subsets sont inclus dans chaque
//     fichier woff2 Zain), mais ne précharge QUE l'arabe (la langue SSR
//     par défaut depuis la DB, defaultCatalogLanguage).
//   - weight:["400","700"] uniquement (weight 300 jamais utilisé, 0 occ.)
//   - preload:true (défaut) → les règles @font-face .p. SONT matchées
//     car il n'y a qu'une seule instance → zéro doublon, zéro double fetch.
//   - Résultat mesuré : 3 fichiers woff2 / 45.1 KiB (vs 6 fichiers / 87.4
//     KiB sur main = −48%), zéro doublon, texte arabe peint en Zain dès
//     le 1er paint, texte latin découvert via CSS (post-paint, FOUT mineur).
const zain = Zain({
  variable: "--font-zain",
  subsets: ["arabic"],
  display: "swap",
