      ...
      if (!isNewDataSource && preservedStockValues.size > 0) {
        ...
        for (const newRow of newRows) {
          ...
          if (preserved) {
            ...
            // ━━━ IMPORT BINARY RULE: stock → Disponibilité ━━━
            // During import, apply strict binary logic:
            //   stock > 0 → Disponible ('true')
            //   stock = 0 → Épuisé ('false')
            // "Sur commande" is NEVER auto-generated by import — purely manual.
            if (preserved.stock > 0) {
              updatedData.__disponibilite__ = 'true'; // Import: stock positif → Disponible
            } else {
              updatedData.__disponibilite__ = 'false'; // Import: stock nul → Épuisé
            }
