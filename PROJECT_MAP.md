...
| FX26 | Fix régressions V4.1.1 — Recherche SQLite | `GET /api/orders` : remplacement du bloc `where.OR` (Prisma `contains` casse-sensible) par `db.$queryRaw` avec `LOWER(CAST(field AS TEXT)) LIKE %q%` — compatible SQLite natif ; scope élargi de 4→6 champs (ajout `customer_address`, `product_price`) ; mapping snake_case→camelCase manuel pour cohérence avec Prisma findMany ; branche `fix/orders-v4.1.1` ; CONTRAINTE : `LOWER()` SQLite ne gère pas les diacritiques (é→E) — limitation documentée | ⏳ CORRIGÉ — EN ATTENTE D'AUDIT |
| FX27 | Fix régressions V4.1.1 — Archivage cancelled | `POST /api/orders/archive` : ajout de `'cancelled'` dans le tableau `{ in: ['delivered', 'confirmed', 'cancelled'] }` ; message d'erreur utilisateur mis à jour pour refléter les 3 statuts éligibles ; branche `fix/orders-v4.1.1` | ⏳ CORRIGÉ — EN ATTENTE D'AUDIT |
| FX28 | Fix régressions V4.1.1 — Badge i18n | `OrderStatusBadge.tsx` : suppression du champ `labelKey` obsolète (clés `order.status*` non existantes), ajout d'une prop `label?: string` (pattern controlled label) ; appelants mis à jour : `OrdersTable.tsx` L.340 et `OrderDetailSheet.tsx` L.124 passent `t(\`adminOrder.status_\${status}\`)` ; branche `fix/orders-v4.1.1` | ⏳ CORRIGÉ — EN ATTENTE D'AUDIT |
| FX29 | Fix régressions V4.1.1 — Clic ligne + stopPropagation | `OrdersTable.tsx` : ajout de `onClick={() => onRowClick(order)}` sur `<TableRow>` L.242 + `cursor-pointer` ; `stopPropagation()` sur Checkbox (L.250) et les 3 Inputs d'édition inline (customerName L.259, productName L.284, customerCity L.314) pour éviter que le clic enfant n'ouvre la side-sheet ; branche `fix/orders-v4.1.1` | ⏳ CORRIGÉ — EN ATTENTE D'AUDIT |
