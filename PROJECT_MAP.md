...
| `DEBT-6` | Haute | UX Contact Modal | **✅ RÉSOLU — Modale de contact intégrée + audit REV1 corrections** (mission `ABAYA-FEATURE-CONTACT-MODAL` + `ABAYA-FIX-CONTACT-MODAL-REV1`, branche `feat/direct-contact-modal` merged). Modale de contact intégrée : `ContactModal.tsx` (shadcn Dialog + Input email + Textarea message + Button submit). Route `POST /api/contact` (publique + rate limiting 60s/IP) stocke en BDD (`ContactMessage` Prisma model). Route `GET /api/contact` (admin only). `CatalogPreview.tsx` : `handleEmailClick` ouvre la modale. 11 clés i18n `contact.*` × 3 locales. **Corrections audit REV1** : A3 (regex email robuste), A4 (`maxLength={2000}`), A5 (`dir={rtl}` explicite), A6 (rate limiting in-memory Map par IP, 429 + cleanup). A1+A2 vérifiés corrects. Branche merged puis supprimée. | `src/components/preview/ContactModal.tsx` ; `src/app/api/contact/route.ts` ; `src/components/preview/CatalogPreview.tsx` ; `src/lib/i18n/dictionaries.ts` ; `prisma/schema.prisma` |
