...
| `DEBT-6` | Haute | UX Contact Modal | **⏳ EN COURS (REV1 — Audit corrections appliquées)** (mission `ABAYA-FEATURE-CONTACT-MODAL` + `ABAYA-FIX-CONTACT-MODAL-REV1`, branche `feat/direct-contact-modal`). Modale de contact intégrée : `ContactModal.tsx` (shadcn Dialog + Input email + Textarea message + Button submit). Route `POST /api/contact` (publique + rate limiting 60s/IP) stocke en BDD (`ContactMessage` Prisma model). Route `GET /api/contact` (admin only). `CatalogPreview.tsx` : `handleEmailClick` ouvre la modale. 11 clés i18n `contact.*` × 3 locales. **Corrections audit REV1** : A3 (regex email robuste `/^[^\s@]+@[^\s@]+\.[^\s@]+$/`), A4 (`maxLength={2000}` sur Textarea), A5 (`dir={rtl ? 'rtl' : 'ltr'}` explicite sur Textarea), A6 (rate limiting in-memory Map par IP, 429 + cleanup). A1+A2 vérifiés corrects (props déjà bien câblées). | `src/components/preview/ContactModal.tsx` ; `src/app/api/contact/route.ts` ; `src/components/preview/CatalogPreview.tsx` ; `src/lib/i18n/dictionaries.ts` ; `prisma/schema.prisma` |
