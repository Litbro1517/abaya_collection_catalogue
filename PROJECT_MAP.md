...
| `DEBT-6` | Haute | UX Contact Modal | **⏳ EN COURS — Modale de contact intégrée** (mission `ABAYA-FEATURE-CONTACT-MODAL`, branche `feat/direct-contact-modal`). Remplace la solution DEBT-5 (clipboard + mailto fallback) par une véritable modale in-app : `ContactModal.tsx` (shadcn Dialog + Input email + Textarea message + Button submit). Route `POST /api/contact` (publique) stocke les messages en BDD (`ContactMessage` Prisma model). Route `GET /api/contact` (admin only, `getCurrentAdmin()` guard) pour consultation. `CatalogPreview.tsx` : `handleEmailClick` ouvre la modale au lieu du fallback mailto. 11 clés i18n `contact.*` × 3 locales (FR/EN/AR). Zéro dépendance ajoutée. | `src/components/preview/ContactModal.tsx` ; `src/app/api/contact/route.ts` ; `src/components/preview/CatalogPreview.tsx` ; `src/lib/i18n/dictionaries.ts` ; `prisma/schema.prisma` |
