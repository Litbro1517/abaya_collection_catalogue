...
---
Task ID: native-color-column-priority
Agent: main
Task: [SURGICAL EDITING] — Priorisation de la Colonne Native de Couleur sur le Catalogue Public (CTC). The public catalog must use EXCLUSIVELY the native color column (validated in admin) as the single source of truth. Replace any reads of old fields or raw import columns (product.couleur, optionscouleurs) with the native engine field. Clean fallback when native column empty (hide block, no broken pills). Harmonize all locations (filters, cart, order recap).

Work Log:
- Delegated codebase exploration to Explore subagent (Task ID: explore-color-arch) which produced a comprehensive 7-section report on the color architecture.
- Key architectural finding: there is NO static Product model. Products are Row records in a dynamic DataSource. The "native color column" is a Column of type 'COLOR' identified by SectionConfig.colorColumn (slug pointer, production value '__colors__'). Values stored as comma-separated color NAMES in row.data['__colors__']. ColorMap (Prisma model) is the centralized {name, slug, hex} library. /api/colormap returns the full list; /api/colormap/lookup?names=... resolves names to hex server-side.
- Identified the "leak": both CatalogPreview.tsx (~line 1466-1502) and ProductPage.tsx (~line 258-291) had a 3-tier fallback chain: (1) config.colorColumn [native, correct], (2) optionscouleurs/option-couleurs/couleurs [raw import, WRONG], (3) variantColumn non-size values [legacy, WRONG]. When the native column was empty, the catalog silently fell back to raw unvalidated import fields — producing broken/missing pills for colors not in ColorMap (e.g. "ZITI", "GROUNA" on "Robe hiba").
- Verified production catalog config via GET /api/catalog: Section has colorColumn='__colors__' configured. Verified production row data via GET /api/datasources/{id}/rows: 6 of 8 sampled products have native __colors__ populated (Kitma montoni: "Gris, Beige, Rose"; Monto mlifa kachmir: "noir, gris, BLANC"; Ensemble chahira: "BEIGE, Noir, BLEU MARINE, VERT D'EAU"; Ensemble robe chemise zara: "BEIGE, Rose mortadila, Noir, BLEU djin"; عباية راقية: "Bleu sail, rose, Blue, Beige, Marron, Black"; انصومبل دوبياس: "Noir, Blanc"). 2 products have EMPTY native column (Robe hiba, عباية بنت حوران) — these currently show broken pills via legacy fallback and will cleanly hide after the fix.
- Edit 1 (Priority 1 — ProductCard): src/components/preview/CatalogPreview.tsx — stripped the optionscouleurs/option-couleurs/couleurs fallback block. Now reads ONLY config.colorColumn. If empty → return null (block hidden). Kept the resolveColorHex(name, colorMapData) hex resolution and the .color-dot-missing class for names not in ColorMap (these still render as neutral dots IF the native column is populated but a specific name isn't in ColorMap — this is a hex resolution concern, not a column source concern).
- Edit 2 (Priority 1 — ProductDetails): src/components/preview/ProductPage.tsx — stripped the entire 3-tier fallback chain (colorNames → fallbackColorNames → legacyColorNames). Removed the fallbackColorKeys array, the fallbackColorValue loop, and the legacyColorNames extraction from variantColumn. Now computes finalColorNames directly from config.colorColumn only. The existing {colorData.length > 0 && (...)} guard at line 797 hides the entire color section when the native column is empty.
- Edit 3 (Priority 2 — clean fallback): No additional code needed. The existing return null (card) and {colorData.length > 0 && ...} (page) already implement the user's "masquez le bloc" requirement. No neutral placeholder, no broken pills.
- Edit 4 (Priority 3 — Merci page harmonization): src/app/merci/page.tsx — added colorHex state + useEffect that fires when order.productColor changes. Fetches /api/colormap/lookup?names={encodeURIComponent(colorName)}, finds the matching entry by name, sets colorHex. Updated the color row to render <span className="checkout-color-chip" style={{backgroundColor: colorHex}} /> + <span>{colorName}</span> when colorHex is present, or just the name when colorHex is null. Mirrors CheckoutPage's recap exactly (same CSS class).
- Edit 5 (Priority 3 — CSS): src/app/globals.css — added .merci-recap-val--color { display: inline-flex; align-items: center; gap: 8px; } mirroring .checkout-recap-val--color for visual continuity between checkout and thank-you screens. The .checkout-color-chip class was already globally defined (16px circle, border, padding-box) and is reused on the Merci page.
- CheckoutPage: no change needed — it already inherits selectedColorHex from ProductPage (which now reads native column only). Cart = CheckoutPage in this app (direct ProductPage → CheckoutPage → Order flow, no persistent cart).
- Color filter: the exploration confirmed NO color filter exists in the public catalog (only category/subcategory filters + search). The user's mention of "filtres de recherche par couleur" is aspirational; no action needed since there's no existing color filter to harmonize.
- Ran bun run lint — clean (no errors).
- Verified dev server (localhost:3000) renders without errors. Dev seed products have no native colorColumn data → color dots correctly hidden (0 dots per card) — this is the expected fallback behavior, not a regression.
- Committed as 1fd7f0d: "feat(color): native colorColumn as single source of truth on public catalog". 4 files changed, 75 insertions, 39 deletions.
- Pushed to main (triggers Vercel auto-deploy).

Stage Summary:
- Native color column (config.colorColumn → '__colors__') is now the SINGLE source of truth for color display on the public catalog. Raw import fields (optionscouleurs, couleurs, option-couleurs) and legacy variantColumn extraction are NO LONGER read.
- Clean fallback: empty native column → entire color block hidden (no broken pills, no neutral placeholder). Verified on dev seed data (0 dots shown).
- Order recap harmonized: Merci page now resolves hex from the stored color name via /api/colormap/lookup and renders a colored chip matching CheckoutPage. Visual continuity between checkout and thank-you screens.
- 4 files changed: CatalogPreview.tsx (stripped fallback), ProductPage.tsx (stripped 3-tier fallback), merci/page.tsx (added hex resolution + chip), globals.css (added .merci-recap-val--color).
